...
**What:** the 21 body-part drawings of the template's first skin: the knight from style frame
SF1, cut into the parts of `docs/art/HERO_TEMPLATE.md`. He is a stand-in for building and
animating the rig at M9, not one of the four heroes (those wait for the story bible).

**Before you start:** SF1 must be approved. Both prompts attach it.

**The pipeline:**
1. Run **Sheet A** (the 10 base parts), then **Sheet B** (the 11 variants), in the same chat as
   your approved SF1.
...
3. **The Art lane cuts them.** Each part is cut out, scaled, and placed on its fixed canvas with
   ...
   `hero-template-arm-front-bent__ai_v1.png`; single-drawing parts use `default`). You get back an
   assembled preview of the knight built from the cut parts, to approve.
...
**Resolution note:** at this sheet size the knight is about 880 px tall, so parts are scaled up
about 1.8× onto the 1600-px canvases. That is fine for a draft. Your redraw, being vector,
exports sharp.

---

## Sheet A: the 10 base parts
