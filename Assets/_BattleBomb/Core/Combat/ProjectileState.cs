...
namespace BattleBomb.Core.Combat
{
    ...
    public readonly struct ProjectileState
    {
        ...
        public ProjectileState(Vector3 position, Vector3 velocity, float damage, Element element, int lifeSteps)
        {
            Position = position;
            Velocity = velocity;
            Damage = damage;
            ...
            Vector3 origin, Vector3 target, float speed, float damage, Element element, int lifeSteps)
        {
            ...
            return new ProjectileState(origin, direction * speed, damage, element, lifeSteps);
        }
    }
}
