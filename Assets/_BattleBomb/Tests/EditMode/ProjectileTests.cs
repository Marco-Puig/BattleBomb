    ...
    public sealed class ProjectileTests
    {
        ...
        public void Flight_is_a_straight_line_at_constant_speed()
        {
            ProjectileState p = ProjectileState.Fired(
                Vector3.zero, new Vector3(10f, 0f, 0f), speed: 8f, damage: 5f, ElementId.None, lifeSteps: 120);

            Vector3 before = p.Velocity;
            p = ProjectileSimulation.Step(p, Dt);
            p = ProjectileSimulation.Step(p, Dt);

            Assert.That(p.Velocity, Is.EqualTo(before), "No homing, no gravity â€” a straight bolt.");
        ...
        {
            ProjectileState p = ProjectileState.Fired(
                Vector3.zero, new Vector3(3f, 0f, 4f), speed: 10f, damage: 5f, ElementId.None, lifeSteps: 120);
        ...
        public void A_bolt_crosses_depth_to_reach_its_target()
        {
            ...
            ProjectileState p = ProjectileState.Fired(
                deepMuzzle, shallowTarget, speed: 8f, damage: 5f, ElementId.None, lifeSteps: 120);
            ...
            Assert.That(hit, Is.True, "Depth is no obstacle â€” that is ranged's identity (Â§2.2).");
        }
        ...
        public void The_hit_test_respects_its_radius()
        {
            ProjectileState p = new ProjectileState(
                Vector3.zero, Vector3.right, 5f, ElementId.None, 60);
        ...
        public void Jumping_over_a_bolt_works()
        {
            ...
            Assert.That(ProjectileSimulation.HitTest(p, airborne, Radius), Is.EqualTo(-1),
                "The bolt flies at firing height â€” jump is the defence (D26).");
        }
        ...
        {
            ProjectileState p = ProjectileState.Fired(
                Vector3.zero, Vector3.right, speed: 8f, damage: 5f, ElementId.None, lifeSteps: 3);
        ...
        {
            ProjectileState p = ProjectileState.Fired(
                Vector3.one, Vector3.one, speed: 8f, damage: 5f, ElementId.None, lifeSteps: 60);
