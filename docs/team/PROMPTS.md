...
Builder first — it needs Unity open. The orchestrator ("BattleBomb Planning") must be running for
...
per lane. The orchestrator is the session named "BattleBomb Planning": it answers cross-lane
...
Then load messaging with ToolSearch (select:SendMessage,ListAgents) and send "BattleBomb Planning":
...
once, one per lane. The orchestrator is the session named "BattleBomb Planning": it answers
...
one per lane. The orchestrator is the session named "BattleBomb Planning": it answers cross-lane
